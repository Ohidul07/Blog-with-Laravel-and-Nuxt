		...
		<design-content>
			<div class="d-flex justify-content-between">
				...
				<div>
					<nuxt-link to="/admin/categories">
						<design-button :has_button_sm="'btn-sm'">Category Lists</design-button>
					</nuxt-link>
				</div>
			</div>
			...
			<div class="add-category mt-2">
				<form @submit.prevent="handleSubmit">
					<design-form type="text" label="Category Name" v-model="categoryData.name"  :hasError="hasError('name')" :helperText="errorMsg('name')"></design-form>
	...
	export default {
		...
		async mounted() {
	    	const categoryId = this.$route.params.id;
	      	await this.loadCategoryDetails(categoryId);
	    },
	    methods: {
	    	async loadCategoryDetails(categoryId) {
		        try {
		        	const res = await this.$axios.$get(`/api/admin/categories/edit/${categoryId}`);
		          	this.categoryData = {
		            	name: res.data.name,
		    ...
		    async handleSubmit() {
		    	...
		    	try {
		    		this.loading = true;
		    		const categoryId = this.$route.params.id;
		    		const res = await this.$axios.$put(`/api/admin/categories/${categoryId}`, this.categoryData);
          			this.loading = false;
		    		this.$notify.success({
